...
- 软件内查看历史日志，支持日志类型、等级/方向、关键字和日历时间范围筛选
- 自动保存上次配置到程序根目录 `settings.json`
- 串口断开自动重连，适合设备供电切换时 COM 口短暂掉线的场景
- 串口连续重连超时后自动重启对应 PnP 串口设备
- 串口持续离线达到独立阈值后自动重启软件或 Windows
...
如果串口连续重连超过设置阈值仍未恢复，GUI 中的“重连超时重启设备”会按当前 COM 口查找设备管理器里的 PnP 串口设备，并执行禁用、等待 2 秒、重新启用。这个动作需要以管理员身份运行软件，否则日志会提示权限不足。

“重连超时重启软件”和“重连超时重启系统”分别拥有独立开关和独立秒数阈值，默认均关闭。软件自动重启后会自动恢复服务，并延续本次连续掉线的计时，因此可以设置为先重启软件、较长时间仍未恢复再重启 Windows。串口恢复成功后，本次掉线的所有计时和单次动作标记都会清零。

自动重启 Windows 使用系统 `shutdown.exe`，触发后等待 5 秒并强制关闭正在运行的应用。启用前请确保其它应用会自动保存重要数据。
